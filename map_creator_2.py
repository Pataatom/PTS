...
class Maintenance:
    ...
    def load():
        ...
        with open("my_stuff/Tree an nature/tree_placement.txt", "r") as f:
            for line in f:
                ...
                tree_group.add(tree)
        with open("my_stuff/Rocks/rock_placement.txt", "r") as f:
            for line in f:
                place_x, place_y = map(int, line.strip('()\n').split(', '))
    ...
    @staticmethod
    def save():
        with open("my_stuff/Tree an nature/tree_placement.txt", "w") as f:
            for tree in tree_group:
                f.write(f"{tree.place_x, tree.place_y}\n")
        with open("my_stuff/Rocks/rock_placement.txt", "w") as f:
            for rock in rock_group:
...
class SomeBar:
    def __init__(self):
        ...
        # ____tree____
        self.tree = pygame.image.load(r"my_stuff/Tree an nature/tree_1.png").convert_alpha()
        self.tree_rect = self.tree.get_rect(topleft=(self.x + 10, self.y))
        self.tree_selected = False
        # ____tree____

        # ____normal_rock____
        self.normal_rock = pygame.image.load(r"my_stuff/Rocks/normal_rocks/normal_rock_1.png").convert_alpha()
